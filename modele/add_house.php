<?php
        ...
        AND isset($_POST['brnb']) AND $_POST['brnb']!=NULL AND $_POST['brnb']!="")
    {
        ...
        if($zip==$_POST['zipcode'] OR $zip2==$_POST['zipcode'])
        {
            ...
            $addH=$DB->prepare("INSERT INTO house(id_user,title,description,id_town,id_area,address,house_type,nbr_people,nbr_room,pictures,picture_1,picture_2) VALUES(:idUser,:title,:desc,:idTown,:idArea,:address,:houseType,:cap,:brnb,:pictures,:photo1,:photo2)");
                $addH->execute(array('idUser'=>$_SESSION['userId'],'title'=>$_POST['title'],'desc'=>$_POST['description'],'idTown'=>$idTown,'idArea'=>$idArea,'address'=>$_POST['address'],'houseType'=>$_POST['house_type'],'cap'=>$_POST['capacity'],'brnb'=>$_POST['brnb'],'pictures'=>$p['0'],'photo1'=>$p['1'],'photo2'=>$p['2']));
